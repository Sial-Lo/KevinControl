...
class _ScanScreenState extends State<ScanScreen> {
      ...
      (results) {
        if (mounted) {
          for (var scanResult in results) {
            if (scanResult.device.advName.startsWith("KevinControl")) {
              onConnectPressed(results[0].device);
            }
          }
  ...
  void onConnectPressed(BluetoothDevice device) {
    ...
    MaterialPageRoute routeLoading = MaterialPageRoute(
      builder: (context) => LoadingScreen(device: device),
      settings: RouteSettings(name: '/LoadingScreen'),
    );

    Navigator.of(context).push(routeLoading);
  }
