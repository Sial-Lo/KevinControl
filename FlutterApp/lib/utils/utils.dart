...
/// Attempts to parse [s] as an [int]. Returns `null` if parsing fails.
int? tryParseInt(String s) => int.tryParse(s);

/// Attempts to parse [s] as an [int]. Returns [defaultValue] if parsing fails.
int parseIntOrDefault(String s, [int defaultValue = 0]) => int.tryParse(s) ?? defaultValue;

/// Parses [s] as an [int], throwing a [FormatException] if the string is invalid.
int parseIntStrict(String s) => int.parse(s);
